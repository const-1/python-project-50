...
@pytest.mark.parametrize("file1, file2, format_name, expected", [
    ('file1_recursive.json', 'file2_recursive.json', 'plain', 'expected_plain.txt'),
    ('file1_recursive.yml', 'file2_recursive.yml', 'plain', 'expected_plain.txt'),
    ('file1_recursive.json', 'file2_recursive.json', 'json', 'expected_json.txt'),
    ('file1_recursive.yml', 'file2_recursive.yml', 'json', 'expected_json.txt'),
])
...
def test_json_format_validity():
    ...
    try:
        ...
        assert isinstance(parsed, list)  # Should be a list of changes
